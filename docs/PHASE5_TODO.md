...
|---|---|---|
| Live integration tests against ArcadeDB | ✅ **SHIPPED 2026-05-20** | `tests/integration/test_arcadedb_integration.py` -- 4 tests (connectivity ping, record iteration, write round-trip, async). The `integration-tests` job in `.github/workflows/ci-cd-pipeline.yml` now boots `arcadedata/arcadedb:24.4.1` as a service container on the Bolt port 26687. |
