...
|---|---|---|---|
| Q1 | For Phase E5 (MAS integration), should Enhanced Cognee be the *only* memory system the Multi-Agent System uses, or share with its existing memory layer? Affects category prefix design and migration strategy. | E | UNANSWERED |
| Q2 | For Phase F3 (multi-tenant), what is a "tenant"? One human user, one organisation, or one application? Affects isolation model and pricing. | F | UNANSWERED |
| Q3 | For Phase F1 (Grafana), do you already run Grafana somewhere, or should the plan include a `docker-compose-monitoring.yml` to spin up Prometheus + Grafana + Loki alongside the main stack? | F | UNANSWERED |
| Q4 | For Phase D2/D3 (performance + load tests), what throughput targets? Suggested defaults: 100 add_memory/sec, 500 search_memories/sec, p95 latency under 200ms. Confirm or override. | D | UNANSWERED |
| Q5 | For Phase E2 (VPS), preferred provider? Hetzner CX22 (4.50 EUR/mo, EU data centre) or DigitalOcean Droplet ($6/mo, multiple regions)? | E | UNANSWERED |
| Q6 | For Phase C (coverage), if a T1 module turns out to be dead code (not imported anywhere in the MCP tool path), delete it OR write tests anyway? | C | UNANSWERED |
| Q7 | For Phase F6 (open-source polish), do you intend to accept external contributors before monetisation? Affects how strict the CONTRIBUTING and CODE_OF_CONDUCT files need to be. | F | UNANSWERED |
| Q8 | For Phase E1 (laptop install), should the installer auto-start the Docker stack on Windows boot (via Task Scheduler) or be manual? | E | UNANSWERED |
| Q9 | For Phase F11 (encrypted secrets), the sops + age toolchain has a learning curve. Alternative: just use Docker secrets + a locked-down `.env` (chmod 600). Pick one. | F | UNANSWERED |
