...
echo "[INFO] Starting PostgreSQL restore from backup: $BACKUP_ID"
...
result = manager.restore_postgres('$BACKUP_ID')

if result['status'] == 'success':
    print('[OK] PostgreSQL restored successfully')
    sys.exit(0)
else:
    print('[ERROR] PostgreSQL restore failed')
